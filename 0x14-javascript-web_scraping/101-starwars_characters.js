...
//  the req module to perform an HTTP GET request to the Star Wars API URL.
request(apiUrl, function (error, response, body) {
  // Check if there was no err during HTTP request
  if (!error && response.statusCode === 200) {
    ...
    const characterPromises = movieData.characters.map((characterUrl) => {
      return new Promise((resolve, reject) => {
        // Use another req to fetch the data for the individual character.
        request(characterUrl, function (charError, charResponse, charBody) {
          ...
          if (!charError && charResponse.statusCode === 200) {
            ...
            // Resolve the promise with name of the character.
            resolve(characterData.name);
          } else {
            // reject promise with the error message if  there was an error during the HTTP request
